 ...
 *    topic1,topic2
 ...
 sh spark-submit --class spark.streaming.demo2.Kafka_demo_offset \
...
object Kafka_demo_offset {
  ...
  def main(args: Array[String]) {
    ...
    val offset = Map(new TopicPartition(topics,0) -> 105.toLong,
      new TopicPartition(topics,1) -> 105.toLong,
      new TopicPartition(topics,2) -> 105.toLong)
    ...
    val stream = KafkaUtils.createDirectStream[String, String](
      ssc,
      PreferConsistent,
      Subscribe[String, String](topicsSet, kafkaParams, offset)
    )
    // Get the lines, split them into words, count the words and print

    println("directStream")

    stream.map(iter =>(iter.value(), iter.offset() ))
